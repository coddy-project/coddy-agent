    ...
    case $state in
        ...
        argument)
            case $words[1] in
                ...
                serve)
                    _arguments \
                        '1: :((setup\:"enable and check the systemd user service" status\:"report the background dispatcher" stop\:"stop the background dispatcher" restart\:"restart the background dispatcher" set-password\:"write the web UI sign-in account into config.yaml"))' \
