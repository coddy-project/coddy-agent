...
- **`make test-perf`** - the performance group: the benchmarks in the `perf_test.go` files, which `make test` never runs. They time what a turn repeats (rules discovery, the look for the rules a tool call brings in, the send boundary) and report what a session over this repository's own rules costs the context (`system-tokens`, `request-tokens`, `duplicate-rules`). `BENCH=<regexp>` narrows them, `BENCHTIME=5x` sets the runs; compare a branch with `main` before and after a change to the prompt or the rules.
