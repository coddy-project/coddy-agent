...
| **`e2e_subagents`** | **`httpserver/http_e2e_subagents.py`** (trust route, `spawn_agent` run as an `agent` task, read-only child transcript, `include_subagents`, catalog) | **`acp/acp_e2e_subagents.py`** (`coddy agents trust`, persisted `agent` task plus `sub_*` child bundle with the parent link) |
...
**`test_httpserver.sh`** order: **`http_smoke_gateway`**, **`http_e2e_scheduler_api`** (REST CRUD plus on-disk **`$CODDY_HOME/scheduler/*.md`**), **`http_e2e_models`**, **`http_e2e_web`**, **`http_e2e_todo`**, **`http_e2e_memory`**, **`http_e2e_skills_slash`**, **`http_e2e_rules`**, **`http_e2e_background`**, **`http_e2e_subagents`** (`POST /coddy/subagents/marker-reporter/trust`, `spawn_agent` run as an `agent` task with its `sub_*` child session, read-only child transcript, `include_subagents`, catalog), **`http_e2e_toolcalls_persist`**, **`http_e2e_compact`**, **`http_e2e_scheduler_agent`**, **`http_e2e_plan_files`** (plan mode **`plan_write`** to **`plans/e2e-plan.plan.md`**, then **`metadata.runPlanSlug`**), **`http_e2e_ask_mode`** (**`model: ask`** reads the note and refuses the write, **`model: agent`** on the same session writes it), **`http_e2e_config`** (staged uci-like config edit, Russian confirm-commit, rollback from the snapshot; the server config ends the script unchanged), **`http_e2e_remote`** (auth gate, config redaction, workspace parity, local fallback), **`http_e2e_background_reap`** (a background process that outlived a killed coddy is found and reaped). All steps run every time and need a working models backend where the LLM is called.
...
Order: **`acp_smoke_gateway`**, **`acp_e2e_models`**, **`acp_e2e_web`**, **`acp_e2e_todo`**, **`acp_e2e_skills_slash`**, **`acp_e2e_rules`**, **`acp_e2e_config`** (staged config edit into a temp config copy, Russian confirm-commit, rollback), **`acp_e2e_memory`**, **`acp_e2e_background`**, **`acp_e2e_subagents`** (`coddy agents trust` before the spawn, persisted `agent` task plus `sub_*` child bundle linked to the parent), **`acp_e2e_toolcalls_persist`**, **`acp_e2e_compact`**, **`acp_e2e_scheduler_agent`**, **`acp_e2e_plan_files`** (plan file on disk plus run via **`_meta.coddy.dev/runPlanSlug`**), **`acp_e2e_ask_mode`** (**`session/set_mode`** **`ask`**: read-only tool calls and no artifact, then **`agent`** writes it), **`acp_remote`** (the ACP client against a remote `coddy http`).
