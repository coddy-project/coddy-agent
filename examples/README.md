...
| **`gateway/`** | **`tg_e2e_offline.sh`**: the Telegram bot against the fake Bot API and scripted model of **`cmd/tgfake`**, no Telegram and no LLM involved (bash, Git Bash on Windows included). |
...
./examples/gateway/tg_e2e_offline.sh              # boots tgfake --llm and coddy serve --gateway, sends "hello", checks the reply
TG_E2E_KEEP=1 ./examples/gateway/tg_e2e_offline.sh   # leaves both running and prints the chat page URL
