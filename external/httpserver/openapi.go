		...
		"paths": map[string]interface{}{
			...
			"/coddy/providers/models": map[string]interface{}{
				"post": map[string]interface{}{
					...
					"description": "Fetches the model list advertised by a provider row posted in the request body (**`{name, type, api_base, api_key, api_key_command, proxy}`** - the `providers[]` shape). Unlike the GET route, the row need not be saved in config.yaml: the settings form sends the entry being edited, so a provider that only exists in the form (for example right after an OAuth device sign-in) lists its models the same way a stored one does. Fields the body leaves empty are inherited from the saved provider of the same name when there is one, so a sparse **`{name}`** post resolves the stored credentials without secrets travelling over the wire. The credential pair (**`api_key`** / **`api_key_command`**) is one slot: it is inherited only when the body posts neither field, and only while the resolved **`api_base`** still matches the saved row, so an overridden endpoint never receives stored credentials. A posted **`api_key_command`** is executed server-side, exactly as it would be for a saved provider. Returns the same **`{ok:true, models:[{id,name}]}`** / **`{ok:false, error, models:[]}`** shape as the GET; a malformed body or an invalid provider row returns 400.",
