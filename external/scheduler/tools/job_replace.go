	...
	return &tooling.Tool{
		Definition: llm.ToolDefinition{
			...
			Description: "Replaces ALL fields of an existing scheduler job (PUT semantics). Requires job_id in the payload path sense (tool argument job_id matches file). " +
				"Dangerous overwrite of description, schedule, paused, cwd, model, mode, agent, permission_mode and body fields. Prefer coddy_scheduler_job_patch when only a few knobs change.",
