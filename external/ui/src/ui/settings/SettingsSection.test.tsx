...
// Context compaction is part of the ReAct loop: its block renders inside the
// ReAct agent tab, and each block still edits its own top-level config key.
const agentSection: SectionDescriptor = {
  id: "agent",
  label: "ReAct agent",
  kind: "object",
  schemaKey: "agent",
  extraKeys: ["compaction"],
};
...
const agentSchema: JsonSchema = {
  ...
  properties: {
    ...
    compaction: {
      ...
      properties: {
        threshold_percent: { type: "integer", title: "Auto threshold (%)" },
      },
    },
  },
};

function AgentHarness(props: { onDoc: (doc: Record<string, unknown>) => void }) {
  const [doc, setDoc] = React.useState<Record<string, unknown>>({
    agent: { max_turns: 35 },
...
test("the ReAct agent tab renders the context compaction block and edits its own key", async () => {
  let last: Record<string, unknown> = {};
  render(<AgentHarness onDoc={(d) => (last = d)} />);

  const block = screen.getByTestId("settings-block-compaction");
  expect(block.id).toBe("settings-block-compaction");
  expect(block.textContent).toContain("Context compaction");
  expect((screen.getByLabelText("Max turns") as HTMLInputElement).value).toBe(
    "35",
  );

  fireEvent.change(screen.getByLabelText("Auto threshold (%)"), {
    target: { value: "70" },
  });
  await waitFor(() => {
    expect(last.compaction).toEqual({ threshold_percent: 70 });
  });
  expect(last.agent).toEqual({ max_turns: 35 });
});
