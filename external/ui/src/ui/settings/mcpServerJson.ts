 ...
 *  `labelKey` is resolved via translate() at render time so a locale switch
 *  re-renders the options. */
export const PROJECT_TRUST_OPTIONS: Array<{
  value: ProjectTrust;
  labelKey: string;
}> = [
  { value: "ask", labelKey: "mcp.trustOption.ask" },
  { value: "allow", labelKey: "mcp.trustOption.allow" },
  { value: "deny", labelKey: "mcp.trustOption.deny" },
];
 ...
 */
export function showsTrustControl(
  row: MCPServerRow,
  policy: ProjectTrust,
): boolean {
...
export function declarationFacts(
  ...
  const out: Array<{ label: string; value: string }> = [
    { label: translate("mcp.fact.transport"), value: row.transport },
  ];
  // stdio starts a process; the remote transports open a connection.
  if (row.command) {
    out.push({
      label: translate("mcp.fact.runs"),
      value: [row.command, ...(row.args ?? [])].join(" "),
    });
  }
  if (row.url)
    out.push({ label: translate("mcp.fact.contacts"), value: row.url });
  const envKeys = Object.keys(row.env ?? {}).sort();
  if (envKeys.length > 0) {
    out.push({ label: translate("mcp.fact.env"), value: envKeys.join(", ") });
  }
  const headerKeys = Object.keys(row.headers ?? {}).sort();
  if (headerKeys.length > 0) {
    out.push({
      label: translate("mcp.fact.headers"),
...
export function validateMCPServerName(name: string): string | null {
  if (!name.trim()) return translate("mcp.validation.nameRequired");
  if (name.includes("__"))
    return translate("mcp.validation.noDoubleUnderscore");
  if (/[\s/\\]/.test(name)) {
    return translate("mcp.validation.noSpacesOrSeparators");
  }
  return null;
}
...
export function parseServerEntryJson(text: string): {
  ...
  try {
    ...
    return { error: translate("mcp.parse.invalidJson") };
  }
  if (typeof raw !== "object" || raw === null || Array.isArray(raw)) {
    return { error: translate("mcp.parse.mustBeObject") };
  }
  const obj = raw as Record<string, unknown>;
  const entry: MCPEntryJson = {};
  if (obj.type !== undefined) {
    if (typeof obj.type !== "string") {
      return { error: translate("mcp.parse.typeString") };
    }
    entry.type = obj.type;
  }
  if (obj.command !== undefined) {
    if (typeof obj.command !== "string") {
      return { error: translate("mcp.parse.commandString") };
    }
    entry.command = obj.command;
  }
  if (obj.url !== undefined) {
    if (typeof obj.url !== "string") {
      return { error: translate("mcp.parse.urlString") };
    }
    entry.url = obj.url;
  }
  if (!entry.command?.trim() && !entry.url?.trim()) {
    return { error: translate("mcp.parse.commandOrUrlRequired") };
  }
  if (obj.args !== undefined) {
    if (!isStringArray(obj.args)) {
      return { error: translate("mcp.parse.argsStringArray") };
    }
    entry.args = obj.args;
  }
  if (obj.env !== undefined) {
    if (!isStringMap(obj.env)) {
      return { error: translate("mcp.parse.envStringMap") };
    }
    entry.env = obj.env;
  }
  if (obj.headers !== undefined) {
    if (!isStringMap(obj.headers)) {
      return { error: translate("mcp.parse.headersStringMap") };
    }
    entry.headers = obj.headers;
  }
  if (obj.disabled !== undefined) {
    if (typeof obj.disabled !== "boolean") {
      return { error: translate("mcp.parse.disabledBoolean") };
    }
    entry.disabled = obj.disabled;
  }
  if (obj.disabledTools !== undefined) {
    if (!isStringArray(obj.disabledTools)) {
      return { error: translate("mcp.parse.disabledToolsStringArray") };
    }
