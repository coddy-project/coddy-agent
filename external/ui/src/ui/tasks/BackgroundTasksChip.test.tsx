...
test("a subagent waiting for an answer takes over the chip", () => {
  ...
  expect(chip.querySelector(".bgtask-dot--warning")).not.toBeNull();
});

test("the waiting count declines by number in Russian", () => {
  setLocale("ru");
  ...
  expect(screen.getByTestId("bgtask-chip")).toHaveTextContent(
    "3 субагента ждут вашего ответа",
  );
});
