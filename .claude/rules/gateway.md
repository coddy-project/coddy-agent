...
`markdown.go` is the gateway's **outbound rendering step**, and the only place Telegram's syntax exists: the prompt and the transcript carry none of it, so a chat conversation is an ordinary session and the next integration renders the same answer its own way by adding a file like this one.
