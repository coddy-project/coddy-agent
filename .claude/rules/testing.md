...
- Express run (what agents run before finishing): **`make test`**
  - Runs **`make test-opencode-rules`**, **`make ui-build`**, then one **`go test -tags=http,ui,scheduler,memory,cli,gateway,swarm ./...`** - the whole tree with every optional module compiled in, which is what the shipped binary contains. Minutes, not tens of minutes.
- Tag matrix (CI's job, not a local step): **`make test-matrix`** covers every combination - the untagged build with its stubs, single tags, pairs, the shipped set - in four groups, **`test-matrix-lean|http|ui|full`** (the **`TEST_TAG_SETS_*`** lists in **`Makefile`**). **`.github/workflows/tests-on-pr.yaml`** runs the groups as parallel jobs, none cancelled by another's failure, and its **All checks** job waits for every one of them; **`make -j4 test-matrix`** does the same locally. Locally, a build-tag boundary change (a **`_stub.go`**, an **`Available`** const, a **`//go:build`** line) is checked with that one combination: **`go test -tags=<set> ./...`**. The untagged build is also compiled by **`make lint`**, so a signature a stub no longer satisfies is caught before the push.
