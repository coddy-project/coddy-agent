...
- **Subagents** tab (**`SubagentsSection`**, section kind **`subagents`** in **`settingsSections.ts`**, keeping **`schemaKey: "subagents"`**) is **hybrid** like Skills: the generated **`SchemaForm`** of the **`subagents`** section (**`i18nDomain="subagents"`**) saves with **Save all**, and below it a **`.settings-fieldset`** (**`.subagents-catalog-box`**, legend **Definitions**, **`data-testid="subagents-catalog"`**) is **API-driven** over **`GET /coddy/subagents`** and **`POST /coddy/subagents/{name}/trust|untrust`** and never touches the document. The catalog is asked about the **workspace of the session on screen** (**`workspacePath`**, from **`workspaceCtx.path`** in **`App.tsx`**), because receipts are keyed by workspace and **`spawn_agent`** checks the session's own cwd; the resolved path is printed as **`Approvals are recorded for <code>…</code>`** (**`.subagents-workspace`**), followed by a pending hint counted with **`tp`** (**`subagents.catalog.pendingHint`**). Reference capture: **`docs/assets/subagents/settings-subagents-catalog-dark-1280.png`**.
  - **Rows reuse the MCP chrome** (**`.mcp-list`**, **`.mcp-list-item`**, **`.skills-list-item-name|badge|desc`**): the name, a scope badge (**built in** / **yours** / **from the project**), **hidden**, and an amber **needs approval** badge (**`.subagents-badge-pending`**). The description is plain text and is **withheld** on an unapproved row (*Description withheld until you approve this file.*) - the model's prompt withholds it too, and the operator's screen must not be the place a hostile checkout's instructions are rendered first.
  - **Shield**: the MCP **`IconShield`** (exported from **`MCPSection.tsx`**), **`.settings-btn-approve`** amber while unapproved, **only** on a project-scope, non-built-in row under **`project_trust: ask`** (**`showsSubagentTrustControl`** in **`settings/subagentCatalog.ts`**); its title names the digest once approved and it withdraws on the next click. A click re-reads the catalog without the **Loading…** placeholder (**`load(false)`**), so the list never collapses; a refused call reports **`subagents.error.trust`** in **`.settings-error`** and leaves the list as it was.
  - **Declaration note**: an unapproved row expands **`.mcp-trust-note`** with a **`<dl class="mcp-trust-facts">`** from **`subagentApprovalFacts`**: file, model, mode, permissions, tools, denies (only when declared), timeout (**`45s`** / **`10m`**), max turns, runs detached (only when forced), instructions size (**`B`** / **`KiB`**), and the 12-character digest with the full value in its title. A bound the definition leaves out reads as **inherited** (*the parent's model*, *everything the spawning session can call*, …) rather than disappearing, because "restricts nothing" is the fact the decision rests on. Keep these helpers in **`subagentCatalog.ts`** and the routes in **`subagentsApi.ts`** (also used by the transcript notice); do **not** inline them.
  - **Containment**: **`.settings-subagents-section`** gives the catalog box, the list and its rows **`min-inline-size: 0; max-width: 100%`** and the note and its **`code`** **`overflow-wrap: anywhere`**, and widens the facts label column to **104px** (**`flex: 0 0 104px`**) for labels like *permissions* / *инструменты*. An absolute path or a digest must never widen the fieldset past the panel or push the shield out of view.
...
- **Opener** (**`.bgtask-chip`**, **`BackgroundTasksChip.tsx`**) sits at the **end of the transcript**, under the last message and above the composer — not in the nav rail. The tasks belong to this chat and the thing that started them is directly above, so that is where the opener belongs. It reads **`N running tasks`** while work is in flight (**`is-running`**, accent border and tint) and falls back to **`N background tasks`** so the history stays reachable once everything has finished. A chat that never ran anything renders **no chip at all**. When a detached subagent waits for a permission answer the chip says **`N subagents need your answer`** before anything else (**`is-awaiting`**, declared after **`is-running`** so its amber border and tint win, with the **`bgtask-dot--warning`** dot): the panel is closed by default and that prompt is in no transcript, so the chip is the only thing that can raise it.
- **Detached permission prompt** (**`SubagentPermissionCard.tsx`**): a running agent row carrying a usable **`pending_permission`** (**`isAwaitingPermission`**: both the child **`sessionId`** and the **`toolCall.toolCallId`** present) marks its card **`is-awaiting`** (amber border) and renders **`.bgtask-permission`** under the progress bar, and again in the detail pane: a head naming the subagent (**`.bgtask-permission-head`**, *The subagent "writer" is waiting for your approval:*), the same **`PermissionToolPreview`** an inline prompt uses (non-interactive, built by **`buildPermissionToolPreview`** after **`stripSubagentTitlePrefix`** drops the relay's **`[subagent <name>]`** prefix, which the head already says and which would otherwise blank the preview header), and one **`permission-prompt-btn`** per offered option (**`.bgtask-permission-actions`**, wrapping). The answer goes to the **child** session through **`submitPermissionChoice`** (**`chat/permissionSubmit.ts`**, shared with **`PermissionPromptSection`**), then the list is re-read (**`onRefresh`**). A waiting task is still a running one (**`isAwaitingPermission`** requires **`running`**, so a stale prompt on a finished row is ignored), which keeps the poll at its 2.5s cadence while it waits. Reference capture: **`docs/assets/subagents/tasks-detached-permission-dark-1280.png`**.
...
- **Refused spawn approval** - a **failed** `spawn_agent` row whose arguments name an agent renders **`SubagentApprovalNotice`** (**`.subagent-approval-notice`**) **after** the row's `<details>`, so it is seen with the row collapsed. It parses no refusal text: it asks **`GET /coddy/subagents`** (with the session workspace) whether that definition is **`needs_approval`** and renders **nothing** otherwise, including when the catalog cannot be reached. The notice is amber like the approve shield (border **`#ff9f0a`** at 35%, tint at 8% over **`--coddy-blend-base`**), wraps its declaring path (**`.subagent-approval-text`**, **`overflow-wrap: anywhere`**, monospace **`code`**) and carries **Approve** (**`.settings-btn-approve`**) and **Open Subagents settings** (**`#/settings/subagents`**). Approving posts the receipt and replaces the actions with *Approved "reviewer" for this workspace. Ask again to run it — nothing was started automatically.*: the spawn is **never retried** on the user's behalf. A refused approval keeps the button and adds the server's message (**`.subagent-approval-error`**). Reference capture: **`docs/assets/subagents/chat-spawn-refused-dark-1280.png`**.
...
- **A card** — **`li.composer-queue-item`** (**`data-testid="composer-queue-item"`**): **`12px`** radius, a **6%** text tint over **`--coddy-blend-base`** with a **12%** border, **`12px`** of text-side padding, **12px** type at **1.45** line height in a **72%** text tint. The rows stack in **reading order** with a **`4px`** gap: the top card is the one the agent reads first.
