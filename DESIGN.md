...
**`load_skill`** is the exception to the raw-text result. The summary row already names the skill, so the call renders no argument card; the body a **completed** call returns is a skill's markdown and renders through the Markdown component with no **Result** strip above it (**`.tool-call-result-card--plain`**, **`.tool-call-result-content--markdown`**), in the prose font rather than the panel's monospace stack, with list indents tightened for the narrower card. A **failed** call returned an error rather than a skill and keeps the ordinary strip and raw text.
