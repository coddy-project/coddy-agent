...
Coddy's own documentation, read out of the binary (**`GET /coddy/docs*`**), at **`#/docs/<page>#<section>`**. It is a reading surface, so it takes the swarm screen's glass dock (**`.docs-dock-cluster`**, the same fixed inset, radius, border, tint and blur as **`.swarm-dock-cluster`**) and reuses the app's controls rather than drawing its own. Reference capture: **`docs/assets/built-in-docs/reader-page-dark-1280.png`**.

- **Header** (**`.docs-header`**): title **Documentation** and a subtitle naming the version, then **Ask the agent about this page** (**`.docs-ask`**, a **999px** pill on a **16%** accent plate with a **45%** accent border, the accent's own action because it starts a conversation) and the close control, which is the shared **`.sessions-close`** - never a bare glyph. The header is **sticky** and runs the full width of the dock on a **82%** canvas plate with a **12px** blur and a hairline under it; what is below it keeps a centred **1320px** column (**`--docs-gutter`**).
- **Three columns** from **1280px**: the contents (**240px**), the page (a reading measure of at most **780px**), **On this page** (**210px**); below **1280px** the outline goes, below **1200px** (the stacked shell) the contents fold into a **Contents** button with **`<Chevron pointing="down">`** above the page. Both side columns are sticky under the header, scroll on their own with the thin scrollbars of Settings, and pad themselves by **3px** so a focus ring is never clipped.
...
- **Search** (**`.docs-search`**) is the swarm screen's field (**`--coddy-surface-field`**, **10px** radius, **14px** text, a **2px** accent focus outline). While it holds a query the contents give way to the hits (**`.docs-hits`**, a **`role="listbox"`** the input controls as a **combobox**): page **›** section, then a three-line snippet whose matched words are **`<mark>`**s on a **24%** accent plate; the selected hit (**`aria-activedescendant`**) sits on a **14%** accent plate with a **32%** accent border.
