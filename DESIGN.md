...
- **Skills** tab (**`SkillsSection`**) combines the schema-driven **`skills.dirs`** editor, **remote marketplace source management** (add via **`POST /coddy/skills/sources`** with **`sync:true`**; the configured sources list with per-row **Remove** via **`DELETE /coddy/skills/sources?source=`**; **Sync** via **`POST /coddy/skills/sync`**), and the installed-skills list. Each skill row shows its **version** (**`.skills-list-item-version`**) and a **`remote`** badge when synced; enable/disable via **`/coddy/skills/{name}/enable|disable`**, remove synced skills via **`DELETE /coddy/skills/{name}`**. A **Refresh** button (**`IconRefresh`** + label, tooltip) re-lists and calls **`GET /coddy/skills/updates`**; when a skill is behind, a per-row **Update** button (**`IconUpdate`** + label, tooltip naming the target version) appears and calls **`POST /coddy/skills/{name}/update`**.
