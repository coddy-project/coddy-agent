...
- **Sessions** tab (**`SessionsManager.tsx`**, section kind **`sessions`**, id **`sessions_manager`** because **`sessions`** is already the config key for the storage directory and stays in **System**) is the stored history as a **table**, API-driven over **`/coddy/sessions`** and outside **Save all**. Reference capture: **`docs/assets/sessions-management-table-dark-1280.png`**.
  - **Toolbar** (**`.sessions-manager-toolbar`**, wrapping flex): a search field (the same **`q`** filter History uses, debounced **250 ms**) that takes the leftover width, then three **icon** actions (**`.settings-btn-danger.settings-btn-icon.sessions-manager-action`**, 40px square) - **Delete selected (N)** (**`data-testid="sessions-manager-delete-selected"`**, disabled with nothing ticked), **Delete all but the open one** (**`…-delete-others`**, disabled without a **stored** session on screen; a client-only draft is not one), **Delete all** (**`…-delete-all`**). Every one of them goes through the shared **`useConfirm`** dialog with **`variant: "danger"`**.
  - **Icon language**: all three are the **same trash can** (**`IconTrash`**, inline SVG in **`SessionsManager.tsx`**) differing only in the mark inside its body - **check** = the ticked rows, **minus** = everything but the one I am in, **cross** = everything; the unmarked can is the per-row delete. The action's words are its **`title`** and its **`aria-label`**, never a label on the button, so three destructive scopes fit beside the search field instead of pushing it onto its own line. The one thing drawn on a button is the **selection count** (**`.sessions-manager-action-count`**, **`…-selected-count`**, absolute at the top-right corner, **`var(--coddy-on-accent-fg)`** on **`var(--accent)`** - not **`--coddy-blend-base`**, which is transparent in the dark themes), because a tooltip cannot show it at a glance; with nothing ticked the badge is absent rather than a zero.
  - **Table** (**`.sessions-manager-table`** inside **`.sessions-manager-tablewrap`**): tick column, **Conversation** (title with the workspace basename on a muted second line, **`.sessions-manager-cwd`**, full path in the cell title; the open session also carries an accent **`.sessions-manager-badge`** and an **`is-active-session`** row tint), **Model** (**`default`** when the session never overrode **`agent.model`**), **Msgs**, **Tokens** (compact **`1.5k`** / **`2.4M`**, the input/output split in the cell title), **Created**, **Updated** (short date, exact instant in the cell title, **—** when the bundle predates the stamp) and the row trash. Header cells are **`position: sticky; top: 0`**. Numeric columns are right-aligned with **`font-variant-numeric: tabular-nums`**.
  ...
  - **Scopes**: ticked rows travel as **`{ids}`**; **Delete all** and **Delete all but the open one** travel as **`{scope:"all"}`** (plus **`except`**), resolved **server side** so they mean the whole history rather than the loaded page (**`bulkDeleteBody`**). The server refuses an **`except`** that names no stored session, and sparing a session spares its **ancestors**, so excepting a **`sub_`** child keeps the parent whose tree would have taken it.
