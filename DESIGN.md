...
When **`memory.enable`** is true, each user turn starts a memory subagent in the background task pool. The transcript shows nothing for it: the run is an invisible **`memory_run`** item of the turn (from the `memory_run` events of the stream, `chat/memoryRun.ts`) that the live status line reads (**Working with memory** while it is in flight). Its record is the **Tasks drawer**: an agent task with the label **`memory: <first line of the message>`**, a **memory** badge (**`bgtask-kind-badge`**, keyed on **`agent.system`**) where a `spawn_agent` child shows **agent**, the child's log in the detail pane and **Open transcript** to the read-only child session.
