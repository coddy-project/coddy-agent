...
- **Picker:** segmented control **directly under the theme swatch grid** in **Settings → Appearance** (**`AppearanceLanguagePicker`**, **`data-testid="appearance-language-picker"`**). It renders **Auto** followed by every locale in **`UI_LOCALE_IDS`**; the current registry supplies **English** and **Русский** (**`data-testid="lang-option-auto|en|ru"`**). Options reuse the swatch border/accent treatment (`.appearance-lang-option`, `.is-active`, `data-active-locale="true"`). It lives inside `.appearance-sheet-body`, so it inherits the centered placeholder guard covered by `appearanceSettingsLayout.test.tsx`.
