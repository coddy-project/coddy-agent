...
- **Runs of a job.** A run is a background agent task under the job's own session, so a job's history is the **background tasks panel** (**`BackgroundTasksPanel`**, unchanged) docked in the scheduler cluster where the editor docks (**`.bgtasks-panel.scheduler-runs-dock`**: relative, the editor's width rule, full cluster height; on **`max-width: 1199px`** it covers the list like the editor). Routes **`#/scheduler/jobs/<job_id>/runs`** and **`#/scheduler/jobs/<job_id>/runs/<task_id>`**; the panel's heading reads **Runs of `<job_id>`**, its empty copy says no run happened yet, and its detail pane labels the agent line **Job**. Each job row carries a **Runs** control (**`SchedulerIconRuns`**, a real **`href`** to the runs route, same-tab handler for a plain click) before the run/stop circle, and a **last-run mark** under the description: the panel's **`bgtask-dot`** in the status tone with the status word and the local clock (**`.scheduler-job-row-last-run`**), absent until the first run. The editor footer carries the same Runs control first. Data comes from **`GET /coddy/sessions/{job session}/background-tasks`** on the panel's cadence (2.5 s while a run is in flight, 15 s idle); **Stop** is the session's task stop, **Clear** is **`DELETE /coddy/scheduler/jobs/{job_id}/runs`** (the transcripts go with the records), **Open transcript** opens the run session, whose read-only notice names the job and links back to the runs route instead of a parent chat.
