...
**`#/tasks`** opens the **Tasks** drawer, **`#/tasks/<task_id>`** opens it with that task's detail pane. Implementation lives in **`external/ui/src/ui/tasks/`** (**`BackgroundTasksDrawer.tsx`**, pure helpers in **`taskStatus.ts`**, REST client in **`api.ts`**); the panel reuses the **`.scheduler-dock-cluster`** placement so the desktop dock and the phone insets are the same geometry as **Scheduler**. Both the desktop **`position: relative`** rule and the **`max-width: 1199px`** cluster rule name **`.bgtasks.drawer.bgtask-dock-drawer`** alongside the scheduler selector; without that the panel falls back to the fixed **`.sessions.drawer`** insets and overflows the cluster on phones.

- **Polling, not SSE.** A background task outlives the turn that started it, so the SSE stream cannot keep the drawer honest. The shell polls **`GET /coddy/sessions/{id}/background-tasks`** every **2.5s** while anything runs and every **15s** otherwise (**`tasksPollIntervalMs`**), and the open detail pane refreshes its output on the same tick. A poll that cannot reach the server degrades to a normal error result — the drawer must not emit an unhandled rejection once per tick while coddy restarts.
- **Rows** (**`.bgtask-row`**, the **`session-item`** family) are two lines plus an optional progress bar: a **status dot** (**`.bgtask-dot--running|success|danger|warning|muted`**, the same vocabulary as the MCP server rows; the running dot pulses and the pulse is dropped under **`prefers-reduced-motion`**), the **monospace label**, a **status chip**, then **elapsed · est. · exit code · overdue**. The row body is a real **`href`** to **`#/tasks/<task_id>`** so **middle-click** opens it in a new tab. Running rows carry a **Stop** control reusing **`composer-run-icon--stop`**.
- **Progress** (**`.bgtask-progress`**) is drawn **only** while the task runs **and** the model supplied **`expected_seconds`**. It is progress toward the model's own estimate, clamped at 100%, and it turns amber once the task is **overdue**. With no estimate there is no bar — the UI never implies knowledge it does not have.
- **Ordering** is running first, then most recently started (**`sortTasksForDrawer`**), because the drawer is about what is happening now.
- **Detail pane** replaces the list inside the same panel (**← Back to tasks**), and shows the command, any error, and the captured output in a scrollable monospace box that follows the tail unless the reader scrolls up. Output the in-memory window has dropped is flagged **truncated**; the full log stays in the session bundle.
- **Empty states** distinguish **no session** (*Open a chat to see its background tasks.*) from **no tasks yet**. A cold load of **`#/tasks`** in a fresh tab has no session and shows the former.
- **Nav rail badge** (**`.rail-nav-badge`**) shows the running count over the Tasks glyph, capped at **`9+`**. It is **`background: var(--text)` / `color: var(--bg)`**, not white-on-accent: **monokai**, **nord**, and **rose-pine** ship light accents where white numerals measure about **2:1**. The accent still carries meaning on the pulsing status dot, where hue matters and contrast does not.
