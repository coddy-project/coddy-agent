...
- **Viewport** - Truncated runs ( **`resultWasTruncated`** from list / SSE) also add **`tool-result-viewport tool-result-viewport--tall`**, clipped with **`tool-result-viewport--clip`** or scrollable with **`tool-result-viewport--scroll`** after **Load more**. Short non-truncated runs omit **`--tall`** so height follows content (**no fake tall box**, **no Load more row**). Large **`write`** / **`write_file`** arguments and **`apply_patch`** diffs reuse the same capped viewport: collapsed content is clipped, while expanded content scrolls inside the tool card instead of growing the transcript.
- **Controls** - Persisted result previews keep **Load more results** (**`data-testid="tool-result-more-link"`** ) and **Hide** (**`data-testid="tool-result-hide-link"`** ). Large write arguments use **`more...`** / **`less`** (**`tool-args-more-link`**, **`tool-args-less-link`**), and large diffs use the same labels (**`diff-load-more`**, **`diff-hide-link`**). All are styled as text links (**`tool-result-text-link`**) in **`tool-result-toggle-row`** under their viewport.
- **Full body** - The SPA obtains saved full strings only via **GET `/coddy/sessions/{sessionId}/tool-calls/{toolCallId}`**. **`App.tsx`** wires **`onFetchToolCallFull`** to that endpoint and merges **`result`** into **`fullResultText`** plus full **`args`** into **`argsText`**. Restored **`apply_patch`** and **`write`** cards auto-fetch when the 200-character list **`argsPreview`** is incomplete JSON, so their diff / capped arguments match the live SSE card after reload.
