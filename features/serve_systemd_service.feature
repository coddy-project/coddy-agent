Feature: coddy serve runs as a systemd user service
  ...
  user runs `coddy serve setup` once, as themselves: it checks the configuration,
  ...
  PATH, so setup hands the service the PATH of the shell it ran from.

  Scenario: setup writes and starts the unit for a binary the install script put in place
    Given a Linux account whose coddy runs from "~/.local/bin/coddy"
    And the account has a valid configuration
    When the user runs coddy serve setup
    ...
    And the service gets the PATH of the shell setup ran from
    And systemd reloaded its units, enabled coddy.service and restarted it
    And setup reports the service running and how to read its log

  Scenario: setup enables the unit the package installed
  ...
  Scenario: uninstall removes the service setup wrote and keeps the user's files
    Given a Linux account whose coddy runs from "~/.local/bin/coddy"
    And the account has a valid configuration
    And the user ran coddy serve setup
    ...
    And the PATH setup handed to the service is gone
  ...
  Scenario: the service is restarted when a configuration change moves a listen address
    Given coddy serve was started by the unit setup writes
