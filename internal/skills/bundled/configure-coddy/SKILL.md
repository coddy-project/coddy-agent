...
- `agent` - ReAct loop model, max turns, LLM retry and pacing (`llm_retry_max` with `0` disabling retries, `llm_retry_base_ms`, `llm_min_interval_ms`, `llm_first_token_timeout_ms` for the wait before the first token, `llm_stream_idle_timeout_ms` for a streamed answer that goes silent mid-way), loop protection, and the opt-in wait for a hit usage limit (`wait_for_limit_reset`, off by default, bounded by `wait_for_limit_reset_max_ms`, four hours);
