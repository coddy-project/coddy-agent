...
- `prompts` - system prompt template overrides (`agent_prompt`, `plan_prompt`, `ask_prompt` files inside `dir`) and `per_provider.enable` (default true: the prompt is tuned to the session's model, and `<mode>.<model-slug>.md` / `<mode>.<family>.md` files in `dir` win over the base file);
