...
func (s *wakeFeatureState) refusedSubmoduleCheckout() error {
	root, err := os.MkdirTemp("", "coddy-bdd-submodule-*")
	if err != nil {
		return err
	}
	s.tmpRoot = root
...
func (s *wakeFeatureState) outlivesItsTimeout() error {
	if s.tmpRoot == "" {
		root, err := os.MkdirTemp("", "coddy-bdd-wake-*")
		if err != nil {
			return err
		}
		s.tmpRoot = root
...
func initializeBackgroundWakeScenario(sc *godog.ScenarioContext) {
	...
	sc.Step(`^a background task with notification enabled finishes as "([^"]*)"$`, s.finishesNotified)
	sc.Step(`^a background task with notification disabled finishes as "([^"]*)"$`, s.finishesQuiet)
	sc.Step(`^three background tasks with notification enabled finish together$`, s.threeFinishTogether)
