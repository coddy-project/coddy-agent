...
// Options configures a remote coddy connection.
type Options struct {
...
func (h *Handler) beginTurn(st *sessionState, cancel context.CancelFunc, sender acp.UpdateSender) (*remoteTurn, error) {
	...
	defer h.mu.Unlock()
	if st.turn != nil {
		return nil, fmt.Errorf("remote coddy: the session is busy (another local request is running)")
	}
...
func (h *Handler) HandleSessionSetMode(_ context.Context, params acp.SessionSetModeParams) error {
	if !session.IsValidMode(params.ModeID) {
