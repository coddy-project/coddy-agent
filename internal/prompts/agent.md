...
- Tool output is capped by line limits. If a **`read`** or **`grep`** result ends with a truncation marker, it is partial: page a file with **`offset`**/**`limit`**, or narrow a search pattern/path to see the rest.
