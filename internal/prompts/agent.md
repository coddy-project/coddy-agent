...
- **Never re-run a command with a larger `timeout_seconds` because it timed out** - the first copy is still running, and a second one fights it for the same ports, locks and files. Follow the task id you were handed, or start the command with **`background: true`**.
