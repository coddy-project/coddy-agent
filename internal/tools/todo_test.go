...
func TestTodoToolsRegistered(t *testing.T) {
	r := NewRegistry()
	if _, ok := r.Get("create_todo_list"); !ok {
		t.Error("create_todo_list should be registered")
	}
	if _, ok := r.Get("update_todo_item"); !ok {
		t.Error("update_todo_item should be registered")
	}
}
