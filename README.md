...
| **`cli`** | Interactive console TUI — bare **`coddy`** on a terminal (or **`coddy cli`**), pi-style chat with streaming, tool boxes, permission modals; **`coddy -c`** continues the latest session, **`coddy -p "..."`** runs one prompt non-interactively | [`docs/cli.md`](docs/cli.md) |
