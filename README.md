...
- **Hooks** - your own commands at lifecycle points of a session: a `PreToolUse` hook can deny a tool call whatever the permission mode, approve it past the prompt, rewrite its arguments or add context, a `PostToolUse` hook can run a formatter or an audit log; definitions use Claude Code's `hooks.json` shape (**`~/.coddy/hooks.json`**, **`.coddy/hooks.json`**, and the workspace's **`.claude/settings*.json`** load too), project files need a one-time approval like MCP servers and subagents - see [Hooks](docs/hooks.md)
