...
| **`cli`** | Interactive console TUI — bare **`coddy`** on a terminal (or **`coddy cli`**): chat with streaming, tool boxes, permission modals; **`coddy -c`** continues the latest session, **`coddy -p "..."`** runs one prompt non-interactively. Visual design inspired by the [pi coding agent](https://github.com/badlogic/pi-mono) TUI (MIT, Mario Zechner) | [`docs/cli.md`](docs/cli.md) |
