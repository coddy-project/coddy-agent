...
`coddy codex login | status | logout` is the same flow under the older codex-only command, kept working for existing scripts. It predates `coddy providers`, which covers every backend, so the commands above are the ones to reach for; `coddy codex login --provider NAME` remains the way to sign a codex provider in before `config.yaml` lists it.
