...
| `wait_for_limit_reset_max_ms` | int | no | `14400000` (4 h) | Longest time one turn spends waiting for limits in total, the retry wrapper's own sleeps on a limit included; a pause that would exceed it ends the turn at once with the error. An explicit `0` never waits; negative values are rejected. |
