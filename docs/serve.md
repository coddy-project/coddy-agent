...
the directories and files the configuration names. Exit status 1 when a probe fails. The
report and its rules are described in
...
| Change | Effect |
|--------|--------|
| models, providers, skills, permissions, most settings | the live configuration is swapped; `GET /coddy/events` carries `config_reloaded` and open clients re-read (see [the SPA notes](ui.md)) |
