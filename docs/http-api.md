...
| POST | **`/v1/responses`** | **`model`**, **`input`**, optional **`stream`**, optional **`attachments`** ( **`path`** workspace-relative under session cwd, **`agent`** / **`plan`** only; file bodies are hydrated as text - a file stored in another detected encoding, such as Windows-1251, is converted to UTF-8; server rejects traversal, oversized, non-text (binary or undetectable encoding), and folder **`path`**), optional **`inline_files`** (array of **`{name, data_url}`**; **`data_url`** is a `data:<mime>;base64,...` URI; works for all modes — direct YAML model and **`agent`** / **`plan`**). For **`agent`** / **`plan`**: each file is saved to the session **`assets/`** directory (`~/.coddy/sessions/<id>/assets/`) with **`0o444`** (read-only) permissions; the model receives a `<coddy_session_assets>` annotation in the user message so it can `read` or `cp` the files without re-transmitting their bytes over the API. For direct YAML model: each entry becomes an image content part on the user message so multimodal models receive the file inline. **`input`** remains the full composer text including **`@path`** echoes. Keeps history between turns when using headers. **`409`** when another **agent** or **plan** turn already holds the session turn lock. |
...
| GET | **`/coddy/providers/{name}/models`** | Fetches the provider model list. OpenAI, Anthropic, and NeuralDeep use their documented model endpoints; Codex uses the fixed official Codex backend with the saved ChatGPT OAuth token. Credentials and **`proxy`** apply server-side. **`200`** **`{"ok":true,"models":[{"id","name"}]}`** on success; **`200`** **`{"ok":false,"error":"...","models":[]}`** when upstream fails; **`404`** for an unknown saved provider. |
...
Equivalent to **`coddy acp`**: **`--config`**, **`--home`**, **`--cwd`**, **`--sessions-dir`**, **`--session-id`**, **`--log-*`**, optional **`--scheduler-enabled`** and **`--mcp-project-trust ask|allow|deny`** (overrides **`mcp.project_trust`** for this process; see [mcp-integration.md](mcp-integration.md)). Networking flags: **`-H` / `--host`**, **`-P` / `--port`**. Defaults fall back to YAML **`httpserver.host` / port** when left at **`0.0.0.0:12345`**. Auth: **`--auth-token <token>`** (also **`CODDY_HTTP_TOKEN`**) enables bearer authentication without writing the secret to **`config.yaml`**.
