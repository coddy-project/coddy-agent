...
| GET | **`/coddy/mcp`** | Merged MCP server list from three levels: **`mcp_servers`** in **config.yaml** and the user-global **`<home>/mcp.json`** (both scope **`global`**), plus the project-local **`.coddy/mcp.json`** (scope **`local`**); mcp.json files are Cursor-compatible and later levels override earlier ones by name. JSON **`{"object":"coddy.mcp_list","items":[...]}`**; each item has **`name`**, **`source`** (**`global`** / **`local`**), **`origin`** (**`config`** / **`home`** / **`project`** — the owning file), **`readonly`** (**`true`** for config.yaml entries), **`transport`**, **`command`**, **`args`**, **`url`**, **`env`**, **`enabled`**, **`status`** (**`connected`** / **`error`** / **`disabled`** / **`unsupported`**), **`error`**, **`tools`** (**`[{name, description, enabled}]`**), **`disabled_tools`**. Enabled servers are probed for tools over their transport (**`stdio`** spawn, **`http`** streamable with legacy-SSE fallback, **`sse`**; connect, **`tools/list`**, close) with results cached until the definition changes; **`?refresh=1`** re-probes. Unknown transport types report **`unsupported`** without probing. |
