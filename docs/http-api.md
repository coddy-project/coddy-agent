...
| GET | **`/coddy/mcp`** | Merged MCP server list: **`mcp_servers`** from **config.yaml** overlaid with the project-local **`.coddy/mcp.json`** (Cursor-compatible; project entries override by name). JSON **`{"object":"coddy.mcp_list","items":[...]}`**; each item has **`name`**, **`source`** (**`config`** / **`project`**), **`transport`**, **`command`**, **`args`**, **`url`**, **`env`**, **`enabled`**, **`status`** (**`connected`** / **`error`** / **`disabled`** / **`unsupported`**), **`error`**, **`tools`** (**`[{name, description, enabled}]`**), **`disabled_tools`**. Enabled stdio servers are probed for tools (spawn, **`tools/list`**, close) with results cached until the definition changes; **`?refresh=1`** re-probes. Non-stdio transports report **`unsupported`** without probing. |
| POST | **`/coddy/mcp/{name}/enable`** | Clears the server's disabled flag, persisting into the file that defines it (**config.yaml** **`disabled`** or **`.coddy/mcp.json`** **`disabled`**), then reloads config. **`200`** **`{"ok":true}`**; **`400`** for an unknown server. Live sessions pick the change up on their next turn. |
...
| POST | **`/coddy/mcp/{name}/tools/{tool}/enable`** | Removes **`{tool}`** from the server's disabled-tools list (**`disabled_tools`** in YAML, **`disabledTools`** in mcp.json). **`200`** **`{"ok":true}`**; **`400`** for an unknown server. |
...
| PUT | **`/coddy/mcp/{name}`** | Upserts one named entry in **`.coddy/mcp.json`** under the server's default cwd. JSON body is the Cursor-format entry: **`command`**, **`args`**, **`env`** (object), **`url`**, **`headers`** (object), **`disabled`**, **`disabledTools`**. Requires **`command`** or **`url`**; **`400`** when the name contains **`__`**, spaces, or path separators, or the body is invalid. **`200`** **`{"ok":true}`**. Config-defined servers are edited via **`PUT /coddy/config`** instead. |
| DELETE | **`/coddy/mcp/{name}`** | Removes the named entry from **`.coddy/mcp.json`**. **`400`** for config-defined or unknown servers. **`200`** **`{"ok":true}`**. |
