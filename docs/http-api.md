...
| POST | **`/coddy/skills/sync`** | Fetches every source in **`skills.sources`** (GitHub repos, git URLs, or an http(s) **`marketplace.json`** URL) and materializes skills into **`ManagedDir`**. Manual only. **`200`** **`{"ok":true,"added":[...],"updated":[...],"failed":[{"source","error"}]}`**. Invalidates the slash-command cache. |
...
| DELETE | **`/coddy/skills/{name}`** | Removes a remote (synced) skill's directory and its provenance entry. **`200`** **`{"ok":true}`**; **`400`** when **`{name}`** was not installed from a remote source. Invalidates the slash-command cache. |
