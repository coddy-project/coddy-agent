...
| Path | What it holds |
|---|---|
| `session.json` | id, `cwd`, `mode`, the session's model and reasoning overrides, the permission-mode override, the title (derived, or pinned by the user), the `tags` it is filed under and whether it is `archived` (with `archivedAt`), `updatedAt`, the context handed over by `SessionStart` hooks, the run metadata of a scheduler or subagent bundle, and the activity counters behind the unread dot |
