...
Every session is a directory under the sessions root, `$CODDY_HOME/sessions/<id>/` (`~/.coddy/sessions/` by default). The root moves with `sessions.dir` in `config.yaml` or with `--sessions-dir` on `coddy serve`, `coddy acp`, the console and the `coddy sessions` verbs ([config.yaml reference](../reference/config.md#sessions)); the process creates it at start and fails to start when it cannot.

Ids are `sess_` followed by 24 hex characters, whoever started the session: a console run, a browser tab, a chat on a messenger gateway, a subagent run another session spawned, a scheduler job's own session and the runs under it. Where a person was sitting is not something a session id reports, so nothing downstream can branch on it. An id doubles as the folder name, so only letters, digits, `_` and `-` are accepted.

A session spawned by another one is stored inside it, at `<parent>/subagents/<child>/`, and a child of that child one level deeper again, so the sessions root lists the conversations a person started and each bundle carries the work it delegated. A scheduler job's session works the same way: the job's runs are its children. What a bundle is - a chat, a delegated run, a job session, a scheduled run - is in its `session.json`, not in its name.
...
The mode (`agent`, `plan` or `ask`), the model and the reasoning level are session-scoped overrides persisted in `session.json`. The permission mode is a session override too, and outranks `tools.permission_mode` from the configuration, but it is kept in memory only: a restart returns every session to the configured mode (a subagent's child session records its narrowed mode in its own `session.json`, as a record of what it ran under). All four change through one setter - the ACP `session/set_mode` and `session/set_config_option` methods, the composer's selectors (`PATCH /coddy/sessions/{id}` with `selectedModelId`, `selectedReasoning`, `mode` or `permissionMode`), the settings commands on every surface and the console's `ctrl+l` - and a change for the next few turns only is never written to disk ([Session settings](session-settings.md)). The working directory is recorded too: `coddy -c`, the console picker and `coddy sessions list --cwd` filter on it. The web UI picks the folder, the git branch and an optional worktree before the first message and locks them once the transcript has messages ([Web UI](../surfaces/web-ui.md#per-session-workspace-folder--branch--worktree-chips)). The title is derived from the first prompt until you pin one, inline in the chat header or with `PATCH /coddy/sessions/{id}` and a `title`; the same route carries the `tags` and the `archived` flag described below.
...
**Tags** are a short flat vocabulary a session is filed under: lower case, inner whitespace as a hyphen, duplicates dropped, at most eight of at most thirty-two characters each. Nobody has to invent them. The call that names a new chat, `POST /coddy/describe`, asks the model for the phrase *and* one to three topic labels in the same answer, and the `PATCH` that pins the title carries them along - so filing costs no extra request, and a model that ignores the instruction simply names the chat as before. Every first message goes to the model, however short: `git status` is a usable title and no filing at all, and skipping the call for it left the shortest conversations as the only unlabelled ones. They are editable afterwards: `PATCH /coddy/sessions/{id}` with `tags` replaces the set wholesale, an empty array clears it, and omitting the field leaves it alone. The listing filters on them (`GET /coddy/sessions?tags=backend,ui` keeps a session carrying **any** of them), search matches them beside the title and the workspace, and History groups by them.
...
A worktree Coddy opens for a session goes to `<repo>/.coddy/worktrees/<branch>/` - inside the checkout, beside the rest of the project-local Coddy state (`.coddy/rules`, `.coddy/agents`, `.coddy/mcp.json`), the way Claude Code and Codex keep theirs. The branch name is mapped to a folder name by replacing the characters a path cannot hold, so `feature/login` becomes `feature-login`.
...
`.coddy` is repository content, so a checkout can ship it as a symlink pointing somewhere else. A worktrees root that resolves outside the checkout is refused rather than followed, and a branch name that reads as a git option (`--detach`) or maps to no usable folder name is refused before git sees it.
...
`coddy serve` runs one session manager over one store for the HTTP API, the web UI, the Telegram gateway and the scheduler, so a chat with the bot is an ordinary session that the History drawer lists and opens while it is happening, and a turn started on one surface is mirrored into a browser watching the same session ([coddy serve](../operate/serve.md), [Telegram gateway](../surfaces/gateway.md)). The console and `coddy acp` open the same store on their own; with `--remote` they work on the server's sessions instead of a local store. A swarm relay aggregates the session lists of its nodes ([Swarm](../operate/swarm.md)).

A session runs one turn at a time. The turn lock is a file lock on the bundle, so a second process - `coddy serve` while `coddy acp` holds the turn - is refused rather than interleaved (`409` over HTTP), and a cancel also writes a marker into the bundle that the process holding the turn picks up between its polls. Different sessions stream in parallel, each behind its own lock ([Web UI](../surfaces/web-ui.md#parallel-sessions-and-generation-cancel)).
...
`POST /coddy/sessions/{id}/rewind` is refused with **409** while a turn is in flight, and a subagent child session is read-only for it as for everything else ([Web UI](../surfaces/web-ui.md#message-editing-and-history-rewind)). The user-message index counts real user turns only - a compaction summary row carries the user role but is not a turn, and a `background_wake` row is. Every browser tab on the same server hears `session_rewound` on `GET /coddy/events` and refetches; a console attached with `--remote`, an ACP client or a messenger gateway does not subscribe to it and reads the truncated history on its next load. Child sessions spawned by a dropped turn keep their bundles under `subagents/` - the run transcripts stay for audit even though the turn that made them is gone.

## The todo checklist

The `coddy_todo_*` tools (`plan_read`, `plan_replace`, `plan_archive`, `item_add`, `item_update`, `item_remove`, `item_move`, with the statuses `pending`, `in_progress`, `completed`, `failed` and `cancelled`) keep the session's checklist. Every change is mirrored to `todos/active.md` and published to the client as a `plan` update, and while the list is non-empty the system prompt carries a `### Current todo checklist` block that is refreshed before every model call of the turn, so an item ticked mid-turn is visible at once. Ask mode offers no todo tools.

Two rules govern replacement. `coddy_todo_plan_replace` is refused while any item is not `completed` - finish the rows or archive first - and when every row is completed the previous `active.md` moves to `todos/archive/todo-<nanos>.md` before the new list is written. `coddy_todo_plan_archive` marks the open rows completed, writes `todos/archive/plan_<unix_seconds>.md` and clears the active list. Over HTTP the same list is `GET` and `PUT /coddy/sessions/{id}/plan`, and `POST /coddy/sessions/{id}/plan/archive`.
...
A subagent run is a child session: a real bundle under `<parent>/subagents/<child>/` whose `session.json` carries `subagentRun`, `parentSessionId`, `subagentName`, `subagentTaskId` and `subagentDepth`. Children stay out of every default listing - History, `GET /coddy/sessions`, `coddy sessions list`, `coddy -c`, ACP `session/list` - and are read-only transcripts: a prompt, a rewind or a permission answer against one is refused with the parent named ([Subagents](subagents.md#child-sessions)). A scheduler job owns a **job session** (`schedulerRun` and `schedulerJobId` in `session.json`, hidden from the composer list and read-only itself), and every run of the job is a child of it - a child bundle whose `session.json` also carries `schedulerJobId`, `schedulerTrigger` and `schedulerFireSlot`. `scheduler.retain_sessions` keeps the newest finished runs per job and deleting the job deletes its session tree ([Scheduler](../operate/scheduler.md)).
...
```bash
coddy sessions list [--sessions-dir <path>] [--cwd <filter>]
