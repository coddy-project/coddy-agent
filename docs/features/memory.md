...
If you tell it not to consult the notes for this message, it skips recall and answers with one short line. The final text of the pass is merged into the `{{.Memory}}` slot of the system prompt template (the built-in footer of every mode, or `agent.md`, `plan.md`, `ask.md` under `prompts.dir`) together with the session notes ([Configuration](../getting-started/configuration.md)); it lives for that turn only and is not written to `session.json`. The main ReAct loop does not receive the `coddy_memory_*` tool definitions and cannot call memory as a normal tool.
