...
- **HTTP** (`coddy serve`): `GET /coddy/subagents?cwd=<dir>` returns the catalog; `POST /coddy/subagents/{name}/trust` and `POST /coddy/subagents/{name}/untrust` with body `{"cwd": "<dir>"}` write and remove receipts. `cwd` must be absolute and defaults to the server's own working directory. Catalog rows carry `scope`, `trust` (`trusted` / `needs_approval`), the booleans `trusted` and `needs_approval`, `digest`, `path`, `builtin`, `hidden`, and the bounds the definition declares (`tools`, `disallowed_tools`, `permission_mode`, `timeout_seconds`, `max_turns`, `background`, and `role_bytes`, the size of its role body) so an approval surface can show what it is approving. A bound the file does not declare is absent, which means it inherits; the role body itself is never served. Errors are `{"error":{"message"}}` JSON. Details in `docs/reference/http-api.md`.
- **Settings → Subagents** (the web UI): the `subagents` config section as a form, and below it the catalog of the workspace of the session on screen (receipts are keyed by workspace, and `spawn_agent` checks the session's own cwd, so the tab asks about that workspace; the resolved path is printed above the list). Every definition is listed with its scope badge. A project-scope row under `ask` carries a shield button that writes or withdraws the receipt at once and expands the declaration it would cover: the file, model, mode, permission mode, tool lists, timeout, turn cap, whether it always runs detached, the size of its instructions and the digest. An unapproved row shows its name only: the description is where a hostile checkout would put instructions, so it is withheld here exactly as it is withheld from the model's prompt.
- **In the chat**: when the model's `spawn_agent` call is refused, the failed tool row carries an approval notice with **Approve** and **Open Subagents settings**, outside the collapsed row so it is seen. The notice parses no refusal text: the trigger is a failed call named `spawn_agent`, the agent name comes from the call's own arguments, and whether that definition is really awaiting approval is asked of the catalog, so a spawn that failed for any other reason shows nothing. Approving does not retry the spawn; the notice asks the user to ask again, so nothing starts on their behalf.
...
![Settings, Subagents tab: the catalog with a project definition awaiting approval](../assets/subagents/settings-subagents-catalog-dark-1280.png)

*Settings, Subagents tab: the catalog with a project definition awaiting approval and the declaration the receipt would cover*

![A refused spawn_agent row offering the approval](../assets/subagents/chat-spawn-refused-dark-1280.png)

*A refused spawn_agent row offering the approval*
...
`background: true` is a request for a run that outlives the turn that started it, so the gate has to outlive it too. When the spawning turn is gone the relay hands the prompt to a **broker** (`agent.DetachedPermissionBroker`, the same seam shape as `SubagentRuntime`) and blocks on it:

- **`coddy serve` with the HTTP surface** provides one. The runner of the shared session manager hands every turn a slot on `serve.Runtime`, and the HTTP server fills that slot when it comes up (the same way it installs the turn mirror), so a detached child spawned by a web chat, a Telegram turn or a wake reaches whichever HTTP server is up when the child asks. The prompt is published on the **background task row** of the run that is blocked (`pending_permission` on `GET /coddy/sessions/{id}/background-tasks`), the Tasks panel renders it on the task card and in its detail pane with the same preview and buttons as an inline prompt, and the answer goes to `POST /coddy/sessions/{child}/permission`. No route is new: the permission hub is keyed by `(session id, tool call id)` rather than by a live turn, and the handler consults it before the guard that keeps a child transcript read-only. The tasks chip turns amber and says how many subagents need an answer, because the panel is closed by default and the prompt is in no transcript.
- The prompt does **not** take the parent's arbiter slot. That slot exists because the parent holds a single pending record, while a detached prompt hangs on its own task and can wait for minutes; a sibling's live prompt must not queue behind it. The "always" answers are withheld here too.
...
- **The console, `coddy acp`, a scheduled run and a `coddy serve` without the HTTP surface have no broker**: nothing there can show a prompt that outlives a turn. A detached child is refused with the reason above and reports it.

![A detached subagent waiting for an answer on its task card](../assets/subagents/tasks-detached-permission-dark-1280.png)

*A detached subagent waiting for an answer: the amber chip under the chat and the prompt on its task card*
...
- a **detached** child that asks after its spawning turn ended is answered on the server, not by the remote client: the prompt hangs on its task row in the SPA served by that host (Tasks panel), or is answered with `POST /coddy/sessions/{child}/permission` against the `pending_permission` of `GET /coddy/sessions/{parent}/background-tasks`. The remote console and ACP client do not show it, and until somebody answers the run waits up to its hard timeout;
...
The approval surfaces are shown next to the paragraphs that describe them (Settings → Subagents and the refused-spawn notice under [Scopes and project trust](#scopes-and-project-trust), the detached prompt under [Detached runs](#detached-runs-the-prompt-outlives-the-turn)).
