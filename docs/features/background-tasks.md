...
A third kind of task is neither a command nor an agent run: `kind: server` is the static file server the `preview_server` tool starts over a project directory ([Preview server](preview-server.md)). It is goroutines of the Coddy process, launched through the same callback as a subagent run, so it has no pid, no exit code worth showing and nothing for `background_reap` to find after a crash. Its row carries `url`, the address it answers at: while the server runs the Tasks panel shows it as a link on the card itself, with nothing to expand first - the way a subagent run carries **Show transcript** - `/tasks` shows the same row, `background_list` prints the address after the label, and `background_output` is the request log.

It is also the one kind of task with no hard limit. A page the operator asked to keep open has no duration to estimate, so the pool arms no timer for it and `timeout_seconds` reads `0`; only a `timeout_seconds` on the `preview_server` call itself ends it early, verbatim and uncapped by `max_timeout_seconds`. A quiet server is never flagged `silent for …`. Everything else is the ordinary lifecycle: it counts toward `max_concurrent`, `background_stop` and the Stop button end it, deleting the session or draining the process takes it down, and a record left `running` by a dead process comes back as `orphaned`.
...
- The card of the run carries the agent's name as its tag, and on the card itself - no need to open it - the model the child runs on, the tokens its calls have spent, how long it has run and **Show transcript**, which routes to the child session (`#/s/<child id>`), a read-only transcript that stays out of History. A shell task has none of these: it has no model and no transcript, only its elapsed time and, in the open card, the output text.
...
- The preview server is specified in `features/preview_server.feature` (`internal/tools/preview/bdd_preview_server_test.go`: a real pool, a real directory and a real HTTP client), with the refusals - a directory outside the workspace, dot-files, a foreign `Host`, a symlink out of the root - in `internal/tools/preview/preview_test.go` and the untimed task in `internal/bgtask`.
- End-to-end against a real model: `examples/httpserver/http_e2e_background.py`, `examples/httpserver/http_e2e_background_reap.py` (kills its own coddy mid-task and makes a fresh one clean up after it), `examples/acp/acp_e2e_background.py`, and `examples/httpserver/http_e2e_preview_server.py` for the server kind.
