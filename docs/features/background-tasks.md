...
A third kind of task is neither a command nor an agent run: `kind: server` is the static file server the `preview_server` tool starts over a project directory ([Preview server](preview-server.md)). It is goroutines of the Coddy process, launched through the same callback as a subagent run, so it has no pid, no exit code worth showing and nothing for `background_reap` to find after a crash. Its row carries `url`, the address it answers at: the Tasks panel and `/tasks` show it as a link, `background_list` prints it after the label, and `background_output` is its request log.
