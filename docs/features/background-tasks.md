...
- **A detached child that needs a permission answer asks on its row.** Once the spawning turn has ended there is no chat stream left to ask in, so under `coddy serve` with the HTTP surface the prompt is attached to the running agent row as `pending_permission` (the payload of the SSE `permission` event, with the **child** session id, plus `agent_name` and `asked_at`). The panel renders it on the task card and in the detail pane with the prompt's preview and its **Allow** / **Reject** buttons, which answer `POST /coddy/sessions/{child}/permission`; the tasks chip turns amber (`N subagents need your answer`) and the drawer keeps its fast poll cadence while one waits. The waiting task keeps running and keeps its timeout; nothing about the prompt is persisted. See `docs/features/subagents.md` (Detached runs).
...
- Subagent runs on the pool are specified in `features/subagents.feature` (`internal/agent/bdd_subagents_test.go`), `features/subagents_http.feature` (`external/httpserver/bdd_subagents_test.go`, including a detached prompt answered from its task row) and `features/subagents_web_ui.feature` (the prompt answered in the Tasks panel); `Pool.Launch` ordering and `Snapshot.Agent` persistence are unit tests in `internal/bgtask`. See `docs/features/subagents.md`.
