...
- **A click anywhere on a card opens it in place.** There is no second pane: the open card shows the command with a copy button (or **Show transcript** for an agent run), the error if the run ended with one, the captured output in a box that scrolls on its own, and at the bottom how the task ended - the exit code and how long it ran. Open as many cards as you like: each keeps reading its own output while its task runs. Which cards are open is not part of the address, which says only that the panel is showing.
