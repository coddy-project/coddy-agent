...
- **A click anywhere on a card opens it in place.** There is no second pane: the open card shows the command with a copy button (an agent run has no shell behind it and shows nothing there), the error if the run ended with one (not the bare `exit status 2` of a command, which only repeats the exit code), the captured output in a box that scrolls on its own, and at the bottom how the task ended and its exit code (`Failed · Exit code 2`). How long it ran stays on the card's own line, where it is read whether the card is open or not. Open as many cards as you like: each keeps reading its own output while its task runs. Which cards are open is not part of the address, which says only that the panel is showing.
