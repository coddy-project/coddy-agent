...
4. **Before every LLM invocation** inside one **`session/prompt`**, appends the **`<turn_context>`** block after the history (**`internal/agent/turn_context.go`**): the wall clock, the todo checklist after a **`coddy_todo_*`** call, and the rules a filesystem tool call activated since the prompt was frozen. The **`system` message itself is not touched** - rewriting it would throw away the provider's cached copy of the conversation behind it. The exception is a template under **`prompts.dir`** that prints **`{{.UTCNow}}`** or **`{{.TodoList}}`** itself: it is re-rendered per call as before, and gets no block.
