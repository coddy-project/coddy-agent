...
2. Builds the system prompt from **`internal/prompts.Render`**: embedded defaults or files under **`prompts.dir`** named by **`prompts.agent_prompt`**, **`prompts.plan_prompt`**, and **`prompts.ask_prompt`** (defaults **`agent.md`**, **`plan.md`**, and **`ask.md`**). Template data includes **`CWD`**, tools markdown, skills markdown, rules markdown (**`{{.Rules}}`** via **`internal/rules`**), optional **`TodoList`** and **`Memory`**, **`ModelSwitch`** (whether the turn is offered **`switch_model`**; the built-in templates describe the tool only then, so a subagent or a configuration with nothing to switch is not told about it), **`BackgroundWake`** (whether a finished background task can wake the session; where it cannot - a subagent, a scheduled run, `coddy -p` - the templates tell the model to collect results before the turn ends instead of promising a wake), plus **`UTCNow`** (RFC3339 UTC refreshed on every render). Coddy then appends an **`<environment_context>`** block containing **`<os>`**, **`<arch>`**, and the detected **`<shell>`**, even when a custom prompt template is used. The message is rendered **once per turn** and then frozen, so the provider's prompt cache keeps the conversation behind it; the built-in templates render neither **`UTCNow`** nor **`TodoList`**, and what moves during a turn is sent after the history in a **`<turn_context>`** block (**`internal/agent/turn_context.go`**, and *The turn context block* in **`docs/contributing/react-agent.md`**).
3. Prepends that system message to the session message list and appends the newest user turn.
4. **Before every LLM invocation** inside one **`session/prompt`**, appends the **`<turn_context>`** block after the history (**`internal/agent/turn_context.go`**): the wall clock, the todo checklist after a **`coddy_todo_*`** call, the rules a filesystem tool call activated since the prompt was frozen, the memory subagent's report, and the background tasks of the session that are still running. The **`system` message itself is not touched** - rewriting it would throw away the provider's cached copy of the conversation behind it. The exception is a template under **`prompts.dir`** that prints **`{{.UTCNow}}`** or **`{{.TodoList}}`** itself: it is re-rendered per call as before, and gets no block.
...
6. Loops until there are no tool calls, a **`max_turns`** the operator set is reached (0, the default, sets none), the loop guard stops a runaway turn, or cancellation. A turn stopped by **`max_turns`** or **`max_tokens`** leaves a notice that says so in the session's UI log and in **`SessionPromptResult.StopNotice`**.
...
6b. Recovery and transport retries share **`agent.llm_retry_max`** extra attempts per model step (default 3, explicit 0 disables them). The initial request is not charged; tool progress or a new follow-up starts a fresh allowance. Within that allowance, a first-token timeout with no output can be replayed once, and a reply with neither text nor tools can be replayed once then nudged up to twice. Each recovery uses a normal **`max_turns`** iteration; transport retries stay inside an iteration. A reply stopped by **`max_tokens`** is terminal, not an empty-answer recovery. Replays and nudges leave the empty assistant message out of the LLM-facing history while retaining its signed reasoning in the transcript. Loop guards, Stop hooks, fallback models and opt-in quota-reset waits keep their own limits, so this is not a lifetime cap on all LLM calls in a turn.
...
The resilient wrapper (**`resilient.go`**) repeats a call when **`isRetryableLLMError`** says the failure belongs to the attempt, not to the request; each retry draws one slot from the **`agent.llm_retry_max`** shared per-step budget (step 6b above): HTTP **`429`**, **`408`**, **`500`**, **`502`**, **`503`** and **`504`**; a transport failure with no status while nothing reached the caller (an unexpected EOF, a connection reset, an HTTP/2 stream the peer reset - `stream error: stream ID N; INTERNAL_ERROR; received from peer`, as net/http prints it, with no `http2:` prefix - a **`GOAWAY`**, a connection closed because its liveness ping went unanswered, or a stream the idle guard cut before any delta); and a request that never left the client - a TLS handshake that timed out or a dial that did not complete (**`isDialFailure`**: a timeout, no route, a temporary DNS failure). The dial check runs ahead of the **`context.DeadlineExceeded`** gate, because a dial timeout matches that sentinel just like the caller's own timer; the caller's timer is ruled out earlier by the **`ctx.Err()`** check in **`callWithRetry`**. A cancellation, an unknown host and any failure after deltas were emitted stay final.

The transports every provider's HTTP client is built on live in **`transport.go`**, one per proxy setting (an empty **`providers[].proxy`** and **`inherit`** share the one that asks the environment's **`HTTPS_PROXY`**, **`none`** has one with no proxy function at all, every proxy URL one of its own) and shared across the providers built for every turn, so a request reuses the previous turn's TLS session instead of opening one of its own. The requests a row makes besides its completions - the model list, the account usage, a sign-in - take the same transport through **`HTTPClientForProviderProxy`**, so the route of a row is decided in one place. They differ from **`http.DefaultTransport`** in two ways. HTTP/2 liveness: a connection that carried no frame for 30 s is asked for a PING, and one that does not answer within 15 s is closed, so a tunnel or a proxy path that died without a FIN or a RST surfaces as `http2: client connection lost` and is retried, instead of a request that waits forever - a proxy in the path (`HTTPS_PROXY`, **`providers[].proxy`**) answers TCP keepalives itself, and only an end-to-end frame proves the far side is there. And the stall guard: with **`agent.llm_stream_idle_timeout_ms`** set (five minutes by default), a `text/event-stream` body that sends nothing for that long after its first bytes is cut with an error **`IsStreamStalled`** recognises. Each provider's stream keeps the text and reasoning already delivered next to that error, exactly as it does for a truncation, and the ReAct loop persists them before ending the turn with the stall named; the wrapper retries the request only while nothing reached the caller. The wait for the first byte is the first-token guard's, and a blocking (**`stream: false`**) answer, which arrives in one piece, is bounded by **`providers[].timeout_ms`** alone.
...
Discovers `.mdc` / `.md` rules from one project folder under session CWD - the first of `.coddy/rules`, the tool-neutral `.agents/rules` (system id `agents-dir`), `.cursor/rules`, `.claude/rules` and `.codex/rules` that holds a rule file, so another agent's mirror of the same rules is never loaded next to it - plus the operator's own **`${CODDY_HOME}/rules`** (system id `user`), which applies in every workspace and loses a file name clash to the project, and nested `**/AGENTS.md` and `**/DESIGN.md` files ([agents.md](https://agents.md/) convention) read for the folders a tool enters. The rules that always apply go into **`{{.Rules}}`** separately from skills, together with the preamble documents: the agent home's **`AGENTS.md`** and **`DESIGN.md`** when they exist, first, then the workspace's pair; that block is rendered once per rules generation of the session (a compaction, a config reload or a workspace switch starts the next one), so it does not move the cached system message. The files of `instructions.files` go into **`{{.Instructions}}`**, minus whatever the preamble already carries. See **`docs/features/rules.md`**.
...
Activation uses globs, **`alwaysApply`**, **`@mention`** and filesystem tool paths (`MatchScoped`: a `read`/`edit`/... targeting a matching file, or a path in a nested `AGENTS.md` subtree, activates the rule); see **`docs/features/rules.md`**. A rule a tool call activates rides in that call's result (**`llm.Message.Rules`**, **`internal/agent/rules_activation.go`**), and a rule a mention brings - one the user names, one a mentioned path activates - in that user message (**`internal/agent/mentions.go`**); neither enters **`{{.Rules}}`**, and each comes once until a compaction folds it away.

### Mentions (`internal/mention`)

The one grammar of **`@`** references (**`grammar.go`**: paths relative, absolute, under **`~`** or above the workspace, quoted paths, line ranges **`:N-M`** and **`#L`**, folders, **`@session:`** / **`@rule:`** / **`@agent:`** / **`@coddy:`**, web pages; every reading of an ambiguous token, longest first, with the one that exists on disk winning), path resolution against a session's workspace (**`paths.go`**), the workspace index for completion (**`index.go`**: **`git ls-files`** in a checkout, a walk elsewhere, cached per root and rebuilt when a mention starts), fuzzy ranking (**`fuzzy.go`**) and the **`<coddy_attachment>`** element a resolved mention is written as, with its display twin **`ForDisplay`** (**`attachment.go`**). Pure: no sessions. **`internal/session/mentions.go`** resolves a prompt's mentions once, when the message is sent - a file, a folder listing, another session's digest, a rule, a subagent, a plan, a page of the built-in documentation, a web page through the fetcher **`internal/tools/web`** registers - and **`mention_search.go`** answers completion for the console, the web UI and a remote console (**`GET /coddy/mentions`**); **`mention_check.go`** runs the same resolver dry, reading nothing, so the web composer highlights only a mention sending would attach (**`POST /coddy/mentions/check`**). **`internal/agent/mentions.go`** renders the attachments into the user message and adds the rules the mentioned paths activate to that same message, so no mention moves the system prompt. The web composer's twin of the grammar is **`external/ui/src/ui/skills/draftAt.ts`**; both are tested against **`internal/mention/testdata/grammar_cases.json`**. See **`docs/features/mentions.md`**.

### Built-in documentation (`internal/docs`)

The pages of **`docs/nav.yaml`** are embedded by **`docs/embed.go`** (package **`docs`** at the root of the tree, the only place the embed pattern can reach them from) and read by **`internal/docs`**: **`Load`** / **`Default`** build the **`Library`** of groups and pages, each page split into headings with the anchors GitHub gives them (**`Anchor`**, which **`internal/docsgen`**'s link check uses too, so an anchor the check accepts is one every reader finds), its links rewritten for a reader outside the repository (**`coddy:<slug>#<anchor>`** between pages, GitHub at the release tag for images and repository files). **`Search`** ranks sections with BM25F over the page title, the section heading and the text, the index built once per process on first use; **`Resolve`** turns every spelling of a page into the page and its anchor; **`Page.Read`** returns a page or a section under a byte budget with the offset to continue at. Pure: it reads only the embedded tree. The consumers are the tools **`coddy_docs_search`** / **`coddy_docs_read`** (**`internal/tools`**), the **`@coddy:`** mention (**`internal/session/mentions.go`**, **`mention_search.go`**), **`GET /coddy/docs*`** (**`external/httpserver/docs_http.go`**), the console's help (**`external/cli/docs_modal.go`**) and **`coddy docs`** (**`cmd/coddy/docs.go`**). See **`docs/features/built-in-docs.md`**.
...
- **`read`**, **`keep_result`**, **`glob`**, **`grep`**, **`print_tree`**, **`websearch`**, **`webfetch`**, **`question`**, **`load_skill`**, **`coddy_docs_search`**, **`coddy_docs_read`** — no shell, no plan or todo tools, no config tools, and no MCP tools
