...
3. Switch between **agent**, **plan**, and **ask** mode if needed (session operating profiles; **ask** is read-only).
...
Volume and environment details for Compose are in [Docker Compose](#docker-compose). On a bare-metal install without **`CODDY_CONFIG`**, the loader prefers **`$CODDY_HOME/config.yaml`** (see **`docs/getting-started/configuration.md`**).
