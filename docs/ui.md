...
- **Language picker:** segmented control **directly under the theme grid** (**`data-testid="appearance-language-picker"`**) with **Auto** (resolves from **`navigator.language`**, stores no cookie) followed by every locale registered in **`locales.ts`**. The current registry renders **English** and **Русский** as **`data-testid="lang-option-en|ru"`**; the active option carries **`data-active-locale="true"`**.
