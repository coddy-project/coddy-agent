...
| Args | Shared static **`PermissionToolPreview`** (no copy / approval actions); large **write** and **apply_patch** bodies retain measured **More…** / **Less** overflow controls |
| Result | **`div.tool-call-result-card`**, **`aria-label="Tool result"`**, with inner **`pre.tool-result-pre`** |
...
| Full text | First result **More…**, or automatic incomplete-args recovery for restored **`apply_patch`** / completed **`write`** cards - **`GET /coddy/sessions/{id}/tool-calls/{toolCallId}`**, using JSON **`result`** and **`args`** (same object includes **`meta`**) |
