...
| A turn nobody typed: a background task the agent started with `notify_on_finish` ended and woke it | the `background_wake` update, then the same wake as a quoted agent text line (`> Woken by a finished background task: bg_3 make test, failed, exit 2, 1m 30s`) at the head of the answer, outside any `session/prompt`; a gated tool inside it asks with `session/request_permission` ([Background tasks](../features/background-tasks.md#waking-the-agent-when-a-task-finishes)) |
