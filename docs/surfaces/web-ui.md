...
| Summary | Same pattern as **thinking** (**`thinking-summary`**, **`thinking-left`**, **`thinking-chevron`**), with **`thinking-label`**, **`.tool-summary-target`** and **`thinking-dur`** inside one non-wrapping **`.thinking-head`**; **`aria-label="Tool summary"`** |
...
| Result | **`div.tool-call-result-card`**, **`aria-label="Tool result"`**, with inner **`pre.tool-result-pre`**; completed structured todo and **`plan_exit`** cards suppress redundant boilerplate results; **`load_skill`** drops the head strip (**`.tool-call-result-card--plain`**) and renders the skill's markdown (**`.tool-call-result-content--markdown`**) |
...
- `assistant_message`
  ...
  - Only the answer that closes the turn carries the action row (**`.msg-assistant-foot`**: the copy control and the timestamp). The answers a turn leaves behind between tool calls render prose alone - a column of identical copy buttons and repeated minutes reads as chrome, not as information. The row appears on the last **`assistant_message`** of the transcript, and only once the turn is over (**`generating`** false), so it never promises a final answer mid-turn.
