...
- The key never enters the settings document or browser; it is stored by the server under **`$CODDY_HOME/providers/<name>/neuraldeep-auth.json`**. Tier models are added under **Logical models**: **Fetch models** there posts the provider row as the form holds it, so the stored login is used even before the row is saved; the CLI flow (**`coddy providers login neuraldeep`**) appends them to the config automatically.
