...
- `http_request` names the address it really fetched, query included, and everything that changes where it went or what it trusted: headers with credential-bearing ones masked, the kind of body, the proxy with its credentials hidden, a skipped certificate check in the failure colour, followed redirects, the timeout, the output file and the rationale. The response - status, masked headers, the body in monospace, the tool's notes - sits under a rule;
...
- `plan_list` / `plan_read` / `plan_write` show the plans, a plan's name and overview over its body, and what was saved; `session_describe` the title, the tags and what changed; the `config_*` tools their answer as fields, with the values the server redacted; the `coddy_memory_*` tools notes as documents and search hits as rows; `keep_result` and `compact_context` their arguments as fields;
...
![MCP calls in the transcript: arguments as fields, a JSON answer as fields under a rule, a Markdown answer as a document](../assets/web-ui/tool-mcp-cards-dark-1280.png)

*MCP calls: the arguments as fields, a JSON answer under them, a Markdown answer below*

![Structured cards on a narrow screen](../assets/web-ui/tool-structured-cards-dark-390.png)

*The same cards at 390px: the bar wraps rather than clipping the address*
...
| Result | Generic output uses **`div.tool-call-result-card`**, **`aria-label="Tool result"`**, with inner **`pre.tool-result-pre`** and no header row; completed structured todo and **`plan_exit`** cards suppress redundant boilerplate results; **`load_skill`** renders Markdown, and supported tools and MCP calls use **`StructuredToolCard`** (**`data-testid="structured-tool-card"`**) |
...
- Generic tool outputs stay raw monospace text; supported document and structured cards render Markdown or fields within **`ToolCallMessage`**.
