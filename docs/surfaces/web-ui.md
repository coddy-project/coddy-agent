...
- Each fetched id is a row with the id, the advertised name, and the **context window** the provider's listing reports (when it does - shown as **`131k`**, the exact count in a tooltip). The **+** glyph at the row's end - the same minimal control History folder headings use - appends **`provider/id`** to **Logical models** in the same unsaved document, so nothing reaches `config.yaml` until **Save all**. An id already listed shows a check instead, and a failed fetch shows its error inline without touching the form. The window the provider reports also drives the model's context metering at runtime when **`max_context_tokens`** is unset, so an added model measures against the real window without pinning a number.
