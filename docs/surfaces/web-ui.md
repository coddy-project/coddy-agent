...
- Verb + target for the current step, and a step counter when the step runs something other than the model - a tool call, the memory run (`2m 05s · 1.2k tokens · Running make test · 45s`). Waiting, thinking and writing are covered by the turn clock. Only the target ellipsizes when space runs out; on a phone the phrase drops under the numbers.
