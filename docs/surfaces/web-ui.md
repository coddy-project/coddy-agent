...
A row wider than the transcript scrolls the whole page sideways on the stacked shell, and whether a row fits is a layout fact jsdom cannot answer. **`external/ui/scripts/phone-overflow-check.mjs`** measures it in a real engine at the verification widths of the grid (**360**, **375**, **393**, **430**, **599**, **600**, **834**, **1199**, **1280px**) in both shipped languages: the page must not scroll sideways, nothing may stick out of **`.messages-inner`** except inside a box that scrolls on its own (a table, a code block), and every tool row keeps its duration inside the row. It exits non-zero on any of them.

It drives **`src/phone-overflow-check.html`**, a stand that mounts the rows that used to overflow - tool rows named after MCP tools, a long command, an answer with a table, a code block, a long link and a long identifier, a user message of one unbroken word - from the real components against the real stylesheet, every tool row open, so it needs a **`vite`** dev server and no backend.
