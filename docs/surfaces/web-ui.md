...
- **Everything that decides what the list shows is one control**: the sliders button at the right end of the search row opens a menu with four sections (**`SessionsFilterMenu.tsx`**). It sits with the search because both narrow the list below; the drawer head keeps only its close button. The sections are flat, with a tick on the current value, rather than nested submenus: a drawer is narrow, and a hover-to-open submenu in it is a worse target than one more line of text.
