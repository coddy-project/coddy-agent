...
- `tool_call`
  ...
  - A path is shown **relative to the session's directory** when that spelling is shorter (`relativeToolTarget`), so a worktree session does not spend every row on the prefix all of them share. Only real paths are rewritten, never a command, a pattern, a url or a name, and the absolute path stays in the row's `title` and in the expanded card. The live status line follows the same rule.
