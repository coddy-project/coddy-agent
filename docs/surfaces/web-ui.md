...
- `tool_call`
  ...
  - An MCP server's answer that is a whole JSON document is shown indented by two spaces instead of as the one line the server sent. It is re-indented as text, so ids, escapes and repeated keys stay exactly as they came; a preview cut short, the output of Coddy's own tools and an answer past a mebibyte are shown as they are.
