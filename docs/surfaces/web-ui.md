...
- **Every task is one card** (`TaskCard`): status dot, a tag (`taskTag`: `shell`, the agent's name, `memory`), the title (`taskTitle`: the command, or the description of an agent run without the `agent <name>:` prefix the pool writes) and a meta line (`taskMetaLine`: elapsed against the estimate while it runs, `Succeeded · 20s · 12:50` afterwards). Running cards stand at the top under no heading of their own - everything above the **Finished N** counter is running - and add Stop and, only when the model supplied `expected_seconds`, a progress bar.
- **The card is one control and opens in place.** The opener button is stretched over the card's summary, so a click anywhere expands it and the summary tints under the pointer; Stop is a sibling above that surface and never toggles the card. The open card shows the command with a copy control (an agent run shows **Show transcript** instead), the error, the output in a box with its own scroll, and once the task has finished a foot with the exit code and the duration (the duration alone for an agent run: the pool's code for it is synthetic). One card is open at a time, carried by `#/s/<id>/tasks/<task_id>`; there is no detail pane.
- **Finished N** is a counter; expanding it shows the same cards, capped at 40 rendered with a note naming what stays on disk, and a finished task that is open brings the section with it. **Clear** drops the finished history for the session.
...
- **external/ui/src/ui/tasks/BackgroundTasksPanel.test.tsx** (one card shape for every task, the tag and the title, the card as one control with Stop apart, expanding in place with the command, copy, output and foot, the finished counter, Clear, Show transcript, empty and error states)
