...
`switch_model`, `http_request`, the `background_*` tools, `preview_server`, `coddy_docs_search`, and `plan_list` / `plan_read` / `plan_write` use structured transcript cards (`StructuredToolCard.tsx`). The model card names the model, reasoning level and turn or session scope. The HTTP card separates the request from the response status, headers and body, masking credential-bearing headers. Background task rows show task ids, states and output; the preview server exposes its address as a link. Documentation hits show their references and open through the documentation reader. Plan lists show one row per plan, plan reads render the Markdown body without YAML frontmatter, and plan writes show the saved plan's identity. Failed calls and incomplete arguments retain the existing raw fallback. Truncated structured results keep the shared More / Less controls and capped viewport.

![Structured tool calls in the dark transcript](../assets/web-ui/tool-structured-cards-dark-1280.png)

*Model, HTTP, background task, documentation search and plan list calls in the transcript*

![Background output, preview server and plan document cards in the dark transcript](../assets/web-ui/tool-structured-details-dark-1280.png)

*Background output, preview server, plan read and plan write calls*
...
| Result | Generic output uses **`div.tool-call-result-card`**, **`aria-label="Tool result"`**, with inner **`pre.tool-result-pre`** and no header row; completed structured todo and **`plan_exit`** cards suppress redundant boilerplate results; **`load_skill`** renders Markdown, and supported tools use structured cards |
