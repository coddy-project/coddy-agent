...
- The key never enters the settings document or browser; it is stored by the server under **`$CODDY_HOME/providers/<name>/neuraldeep-auth.json`**. Tier models are added under **Logical models**: the provider's own edit form carries a **Fetch models** control that posts the row as the form holds it, so the stored login is used even before the row is saved, and each advertised id can be added to Logical models from the returned list; the CLI flow (**`coddy providers login neuraldeep`**) appends them to the config automatically.
...
- A provider's model list is fetched where the provider is configured: open **Settings → LLM Providers**, pick a row, and the **Advertised models** fieldset fetches the list automatically when the form opens (for a saved row) and again on **Fetch models**. The request posts the row exactly as the form holds it to **`POST /coddy/providers/models`** - unsaved edits, a sign-in that exists only on disk, and a provider the document does not have yet all resolve the same way a saved row does - so the list reflects what is on screen, not what is stored.
- Each fetched id is a row with the id, the advertised name, and the **context window** the provider's listing reports (when it does - shown as **`131k`**, the exact count in a tooltip). The **+** glyph at the row's end - the same minimal control History folder headings use - appends **`provider/id`** to **Logical models** in the same unsaved document, so nothing reaches `config.yaml` until **Save all**. An id already listed shows a check instead, and a failed fetch shows its error inline without touching the form. A **`provider/id`** the document lists but the provider no longer advertises is flagged as a warn row (**`.provider-model-row.is-stale`**, amber) with a **−** control that removes it from Logical models. The window the provider reports also drives the model's context metering at runtime when **`max_context_tokens`** is unset, so an added model measures against the real window without pinning a number.
...
- The **Usage limits panel** switch rides beside the **Provider type** picker (only for `neuraldeep` / `codex` / `devin`, the types with a usage source) - `providers[].usage_limits_panel`, on by default, hides the section and stops the reads when off.
- Automated checks: **`ProviderModelsFetch.test.tsx`** (the fetch posts the edited row, add appends `provider/id`, listed ids disable, a stale row carries its minus, errors render inline), **`ModelField.test.tsx`** (the provider/id composition and split), **`SettingsSection.test.tsx`** (the end-to-end wiring inside the providers section, the usage switch beside the type picker for a usage-capable type only), and the API side's `features/provider_models_fetch.feature` with `external/httpserver/providers_models_http_test.go`.
...
- The label sits level with the switch (vertical centres match within 1px, 8px gap). The label cell (**`.settings-switch-field-label-cell`**) carries the label and the field's **`FieldHint`** (the ⓘ every settings field's description now lives in - a scrollable hover tooltip on pointer devices, a bottom sheet on touch-only ones); there is no description paragraph in the form.
...
- Automated checks: **`SwitchField.test.tsx`** (structure and CSS rules), **`SchemaForm.switch.test.tsx`**, **`SkillsSection.autoDiscovery.test.tsx`**. Live check: open a model form at **1280px** and **390px**, and for each **`[role=switch]`** compare **`getBoundingClientRect()`** of the switch and **`.settings-switch-field-label`** (centre delta ≤ 1px).
...
- On **`max-width: 1199px`** (phones and tablets alike) the **Settings** drawer keeps one inline inset, **14px**, for every band it stacks: the **SETTINGS** head, the section tiles, the opened section and the reload / save footer all start and end on the same edge. Check it by measuring `getBoundingClientRect().left` of `.settings-tile` and `.settings-body` against `.settings.drawer` - the two must agree.
