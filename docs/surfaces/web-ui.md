...
- A tool Coddy does not define takes the generic `Running a tool`, so for one an MCP server serves the target carries the identity of the call instead - `Running a tool playwright/browser_navigate` - and what it acts on is read on the transcript row above, which names the server and the tool as well.
