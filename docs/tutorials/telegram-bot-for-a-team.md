...
6. **Use it from the chat** ([Bot interaction model](../surfaces/gateway.md#group-chats)). In a private chat every message is answered. In a group the bot responds only when it is @mentioned, when a message replies to one of its own, or to `/clear`; under `admin` isolation it also ignores everyone outside `admins`. `/agent`, `/plan` and `/ask` switch the session's mode, `/model` opens an inline keyboard over the configured models, `/context` shows what fills the context window, and `/clear` starts a fresh session for that user or chat - the old bundle stays on disk and `coddy sessions list` still shows it. A changed token rebuilds the gateway in place and a flipped `enable` starts or stops it without a restart; when a tap seems to do nothing, raise one component: `coddy serve --log-level "info,gateway.telegram=debug"` ([Debugging a chat](../surfaces/gateway.md#debugging-a-chat)).
