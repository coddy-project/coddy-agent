...
| `skills.sources` | list of strings | [EvilFreelancer/rpa-skills] | Remote skill sources installed on demand with `coddy skills sync` (never fetched automatically). Each entry is a GitHub repo (owner/repo[@ref]), a git URL, or an http(s) URL to an agents-standard marketplace.json. Materialized into ${CODDY_HOME}/skills. Omit the key and Coddy configures EvilFreelancer/rpa-skills, the marketplace the bundled rpa-* skills are published from; an empty list connects no marketplace at all. See https://coddy.dev/docs/skills. |
