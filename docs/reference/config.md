...
| `tools.preview_server` | object |  | The preview_server tool: a static file server the agent starts over a project directory, on a free port, so the operator can open HTML, JS and CSS work in a browser. The server is a background task of the session and runs until it is stopped, the session is deleted or coddy exits. |
| `tools.preview_server.enable` | boolean or null | true | Offer the preview_server tool. The server is a background task, so tools.background.enable: false turns it off as well. |
| `tools.preview_server.host` | string | 127.0.0.1 | Address the server binds, without a port: the port is always a free one. Anything that is not loopback exposes the served directory to whoever can reach that address. |
| `tools.preview_server.public_host` | string | "" | Host written into the URL the agent hands out, for a coddy that runs where the operator's browser is not (a container, a remote machine). Empty uses the bind host. |
