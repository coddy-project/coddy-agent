...
| `/mcp` | console, web UI, Telegram | Console: opens a server list with connection status, tool count, server and tool switches, and project trust controls under `mcp.project_trust: ask`. Web UI: opens Settings → MCP servers, whatever follows the command. Telegram: lists servers and offers enable/disable buttons for trusted entries only, in a group without a mention. | [MCP](../features/mcp.md#management-api-and-ui) |
...
- the Telegram adapter answers its own commands, passes the settings commands but `/permissions` to the session (`isSettingsCommand` in `external/gateway/telegram/bot.go`), and drops any other message that starts with `/`, so `/compact`, `/export` and skills are not reachable there;
...
|---|---|---|
| Console | A fixed client-side list (`resume`, `new`, `theme`, `hotkeys`, `queue`, `usage`, `tasks`, `mcp`, `docs`, `quit`) merged with the rows the server advertises through the ACP `available_commands_update` notification, the settings commands among them (`slashCatalog` in `external/cli/app.go`). | Typing `/` opens the suggestion menu; Enter on a suggestion applies it and submits in one stroke. |
| Web UI | Two groups in the composer: built-ins from `GET /coddy/commands` plus local `/docs` and `/mcp`, and skills from `GET /coddy/slash-commands`, paged and filtered by `prefix`, scoped to the session workspace through `X-Coddy-Session-ID`. A server built-in row carries its `kind` (`setting` or `action`), the argument `hint`, its `aliases`, and for a settings command the `setting` it changes with its `choices` or fixed `value`. | Both server lists are re-read when the server announces a configuration reload (`event: config_reloaded` on `GET /coddy/events`). Picking `/model`, `/reasoning` or `/permissions` opens that selector; `/agent`, `/plan` or `/ask` switches the mode; `/mcp` opens Settings → MCP servers. Other rows insert the plain `/name` token. Once `/compact` is typed, the composer completes its `--model` option and the option's value from the configured models ([Composer command options](../surfaces/web-ui.md#composer-command-options)). |
...
| Telegram | `start`, `help`, `model`, `mcp`, `agent`, `plan`, `ask`, `context`, `resume` and `clear`, registered with `setMyCommands` at startup. | They appear in the client's command menu; `/reasoning`, `/think` and `/nothink` work when typed but are not in the menu. |
