...
| `HTTP_PROXY`, `HTTPS_PROXY`, `NO_PROXY` | `internal/netx/dial.go` (Go's `http.ProxyFromEnvironment`) | The proxy for the HTTP transports `internal/netx` builds when the section they serve sets no `proxy` of its own; an explicit `proxy` replaces them. `http_request` and `webfetch` honour them too, unless a call passes its own `proxy` (or `direct`), and so does every request of an LLM provider row, unless its `providers[].proxy` is `none` or a proxy URL. `ALL_PROXY` is not read. | [Swarm](../operate/swarm.md#encryption-and-proxies), [Provider proxy](../getting-started/configuration.md#provider-proxy) |
