...
| `CODDY_CODEX_BASE_URL` | `internal/llm/codex_auth.go` | Overrides the Codex backend endpoint for the whole process; `api_base` is ignored for `type: codex` on purpose, so a configuration file can never redirect an OAuth token. The account usage read derives its host from the same endpoint. | [Configuration](../getting-started/configuration.md#config-file-location-and-paths) |
...
| `CODDY_DEVIN_API_SERVER_URL` | `internal/llm/devin_auth.go` | The Devin API server every `type: devin` request goes to (the user JWT, the model catalog, chat, the account usage read) and the fallback of the sign-in code exchange, for the whole process. `api_base` is ignored for `type: devin`, so a configuration file cannot redirect a session token. Meant for stands and tests. | [Devin](../features/devin.md#stands-and-tests) |
| `CODDY_DEVIN_WEBAPP_URL`, `CODDY_DEVIN_API_URL` | `internal/llm/devin_auth.go` | The sign-in page host (`https://app.devin.ai`) and the Devin API that exchanges the sign-in code for a session token (`https://api.devin.ai`) during `coddy providers login devin`. Meant for stands and tests. | [Devin](../features/devin.md#stands-and-tests) |
| `CODDY_DEVIN_CLI_CREDENTIALS` | `internal/llm/devin_auth.go` | The Devin CLI `credentials.toml` a `type: devin` provider falls back to when no Coddy-managed login exists. Default: `$XDG_DATA_HOME/devin/credentials.toml` or `~/.local/share/devin/credentials.toml`, whichever exists. | [Devin](../features/devin.md#where-the-session-token-comes-from) |
...
| `npm_config_registry`, `NPM_CONFIG_REGISTRY` | `internal/mcp/pin.go` (`DefaultResolver`) | The npm registry Coddy asks for the current release of an `npx -y <package>` MCP server when it registers one (Settings → MCP servers, `PUT /coddy/mcp/{name}`, `config_set`), read the way npm reads its own setting; `https://registry.npmjs.org` when both are empty. A `.npmrc` is not read. | [MCP servers](../features/mcp.md#pinning-npx-packages) |
| `HTTP_PROXY`, `HTTPS_PROXY`, `NO_PROXY` | `internal/netx/dial.go` (Go's `http.ProxyFromEnvironment`) | The proxy for the HTTP transports `internal/netx` builds when the section they serve sets no `proxy` of its own; an explicit `proxy` replaces them. `http_request` and `webfetch` honour them too, unless a call passes its own `proxy` (or `direct`), and so do every request of an LLM provider row and the Telegram bot, unless `providers[].proxy` or `gateways.telegram.proxy` is `none` or a proxy URL. `ALL_PROXY` is not read. | [Swarm](../operate/swarm.md#encryption-and-proxies), [Provider proxy](../getting-started/configuration.md#provider-proxy), [Telegram gateway](../surfaces/gateway.md#proxy) |
...
| `CODDY_SERVE_ROLE` | `internal/serve/daemon.go` | Set by `coddy serve --daemon` on the processes it re-executes, to `dispatcher` or `worker`, so each half knows which one it is. `service` tells a foreground `coddy serve` that a service manager starts it again, so a configuration change that moves a listen address ends it with status 75 instead of waiting for a manual restart; the systemd unit of `coddy serve install` sets it, and a supervisor of your own may. Do not set `dispatcher` or `worker` by hand. | [Restarting itself](../operate/serve.md#restarting-itself) |
...
| Variable | Read by | Meaning | Documented in |
|---|---|---|---|
| `CODDY_HOOK_LINT`, `CODDY_HOOK_TESTS`, `CODDY_HOOK_SKIP` | `scripts/checks.sh`, run by `.githooks/pre-commit` | The commit gate: `CODDY_HOOK_LINT=0` skips the linter, `CODDY_HOOK_TESTS=fast|full|matrix` adds tests, `CODDY_HOOK_SKIP=1` bypasses the gate. | [Agent notes](../../AGENTS.md) |
| `CODDY_UI_BACKEND` | `external/ui/vite.config.ts` | The `coddy serve` origin the Vite dev server proxies `/v1`, `/coddy`, `/swarm`, `/docs` and `/openapi.*` to while working on the SPA. | [Assets index](../assets/INDEX.md) |
...
| `CODDY_BIN`, `CODDY_CONFIG`, `CODDY_HOME`, `CODDY_CWD`, `SESSION_ROOT`, `SESSION_ID`, `BASE_URL`, `MODEL`, `WORK_DIR`, `CODDY_CHAT_PROFILE`, `NEURALDEEP_API_KEY`, `RPA_API_KEY` | the harness scripts under `examples/` | Which binary, configuration, server and model an end-to-end script drives; each script's docstring lists its own. | [Examples](../../examples/README.md) |
