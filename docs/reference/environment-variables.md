...
| `CODDY_DEVIN_API_SERVER_URL` | `internal/llm/devin_auth.go` | The Devin API server every `type: devin` request goes to (the user JWT, the model catalog, chat) and the fallback of the sign-in code exchange, for the whole process. `api_base` is ignored for `type: devin`, so a configuration file cannot redirect a session token. Meant for stands and tests. | [Devin](../features/devin.md#stands-and-tests) |
