...
Every markdown root is scanned recursively for `.md` and `.mdc` files. Duplicate rule files (same file name, extension included) resolve with precedence: **coddy > agents-dir > cursor > claude > codex > agents**. A project that keeps its rules in `.agents/rules/` therefore overrides tool-specific copies of the same file, and `.coddy/rules/` overrides everything. Nested `AGENTS.md` files are keyed by full path, so they never collapse into each other.
...
rules:
  auto_discover: true
  systems: []   # optional filter: coddy, agents-dir, cursor, claude, codex, agents
```
